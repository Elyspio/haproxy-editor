...
describe("HaproxyConfigurationParser", () => {
	describe("parseFrontends", () => {
		it("parses multiple frontends", () => {
			const conf = {
				frontend1: ["acl acl1 hdr(host) -i example.com", "use_backend backend1 if acl1"],
				frontend2: ["acl acl2 hdr(host) -i test.com", "use_backend backend2 if acl2"],
			};
	...
	describe("parseFrontend", () => {
		it("parses a frontend with acl and mapping", () => {
			const lines = ["acl acl1 hdr(host) -i example.com", "use_backend backend1 if acl1"];
	...
	describe("parseBackends", () => {
		it("parses multiple backends", () => {
			const conf = {
				backend1: ["server srv1 127.0.0.1:8080 check", "server srv2 localhost"],
				backend2: ["server srv3 192.168.1.1:9000 check"],
			};
	...
	describe("parseBackend", () => {
		it("parses backend with multiple servers", () => {
			const lines = ["server srv1 127.0.0.1:8080 check", "server srv2 localhost"];
		...
		it("filters out comments and empty lines", () => {
			const lines = ["", "# this is a comment", "server srv1 127.0.0.1:8080 check"];
			const result = parser["parseBackend"]("backend", lines);
		...
		it("returns empty servers array if no valid server lines", () => {
			const lines = ["", "# comment", "not a server line"];
			const result = parser["parseBackend"]("backend", lines);
